...
- **The sim camera builds under the Jazzy and Humble shells.** Jazzy builds the
  sim stack as packaged; Humble builds it via the `simOverlays` packaging fix in
  `flake.nix` (see the repo-root [CLAUDE.md](../CLAUDE.md) "The sim camera").
  Lyrical and Rolling are excluded — `ffmpeg_image_transport_msgs` fails its
  rosidl build there (beads `ros2_subscribe-lyx`). The flake gates the stack
  behind `simDistros` (`withSim` in [`nix/ros-env.nix`](../nix/ros-env.nix)).
  Run `sim/cam_sim.sh` under `nix develop` (default Jazzy) or
  `nix develop .#humble`; under Lyrical/Rolling the gscam packages are not in the
  closure.
