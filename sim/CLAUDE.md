...
- **The sim camera builds under the Jazzy, Humble, Lyrical, and Rolling shells.**
  Jazzy, Lyrical, and Rolling build the sim stack as packaged; Humble builds it
  via the `simOverlays` packaging fix in `flake.nix` (see the repo-root
  [CLAUDE.md](../CLAUDE.md) "The sim camera"). The flake gates the stack behind
  `simDistros` (`withSim` in [`nix/ros-env.nix`](../nix/ros-env.nix)). Run
  `sim/cam_sim.sh` under `nix develop` (default Jazzy) or
  `nix develop .#humble` / `.#lyrical` / `.#rolling`. On Rolling the camera runs
  but the recorder does not (r2r — beads `ros2_subscribe-2xb`), so Lyrical is the
  one to use for the full sim + recorder workflow.
