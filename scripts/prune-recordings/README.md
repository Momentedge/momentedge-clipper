...
- `prune.sh` — loops: prune, `sleep 60`, repeat. Each pass deletes regular files
  ...
  argument, then `~/edgestream-rec/captured`. A missing directory is skipped (not
...
- `install-remote.sh` — copies the script to a remote host and launches it
  detached (`setsid nohup`), logging to `~/edgestream-rec/prune.log`.
...
```sh
./prune.sh                          # ~/edgestream-rec/captured
...
This prunes `~/edgestream-rec/captured/` every minute. Check on it with:

```sh
ssh melikag@100.67.74.107 'pgrep -af edgestream-prune.sh; tail -f ~/edgestream-rec/prune.log'
```
