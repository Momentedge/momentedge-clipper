...
The recorder installs as `clipper` on PATH (the momentedge-clipper package),
not as an ament executable, so it is an ExecuteProcess rather than a launch_ros
Node. Its mode is a subcommand, so the command run is `clipper tail`.
