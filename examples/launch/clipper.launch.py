...
The recorder installs as `clipper-tailing` on PATH (the momentedge-clipper
package), not as an ament executable, so it is an ExecuteProcess rather than a
launch_ros Node. The package also installs a `clipper` symlink beside it for one
release, so a launch written before the rename still resolves.
...
Run with a sourced ROS 2 environment:
    ...
    ros2 launch examples/launch/clipper.launch.py clipper_bin:=./target/release/clipper-tailing   # dev build

It is launched with respawn=True so it restarts on exit (it re-discovers the
recording).
"""
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument("clipper_bin", default_value="clipper-tailing"),
