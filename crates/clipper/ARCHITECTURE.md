           ...
           + timestamps)  (per channel)   + ended flag)
               ...
               └──────────────┴──────────────┘
                       ...
                       [trigger-<ns> thread]  ◀── /events/momentedge/trigger
                         ...
                         4. publish /events/momentedge/recorded
```
...
4. **Announce.** Publish `/events/momentedge/recorded` — only after the clip is in `out_dir` and `out_dir` is fsynced, so the announced path is always crash-durable.
